...
def load_assets():
    try:
        ...
        # Pre-process Duration to minutes for EDA
...
def predict_price(airline, stops, j_date, d_time, a_time):
    ...
    airline_df = pd.DataFrame(airline_encoded, columns=ohe.get_feature_names_out())
    full_df = pd.concat([input_df, airline_df], axis=1)
    ...
    return np.expm1(model.predict(scaled_data)[0])
        ...
        if model:
            try:
                ...
                if trip_type == "Round Trip":
                    ...
                    with m1: st.metric("Outbound Fare", f"₹{p_out:,.2f}")
                    with m2: st.metric("Return Fare", f"₹{p_ret:,.2f}")
                    with m3: st.metric("Total Estimate", f"₹{(p_out+p_ret):,.2f}")
                else:
                    st.metric("Estimated One-Way Fare", f"₹{p_out:,.2f}")
            except Exception as e:
...
with tab3:
    ...
    if not df_train.empty:
        ...
        e5, e6 = st.columns(2)
        with e5:
            # Fixed scatter plot to use numeric Duration_mins
            st.plotly_chart(px.scatter(df_train, x="Duration_mins", y="Price", color="Airline", opacity=0.4, title="Duration (Mins) vs. Fare Correlation"), use_container_width=True)
        with e6:
            st.plotly_chart(px.pie(df_train, names='Airline', hole=0.5, title="Airline Dataset Distribution"), use_container_width=True)
    else:
        st.warning("Training data missing for EDA.")